...
@section('title')
    | @foreach ($princes as $one) {{ $one->title}} @endforeach
@endsection
...
@endforeach
 ...
 <div class="comments">
    ...
    @else
        @foreach ($all_comments as $one)
            <div class="one-comment">
                    <div><img src="{{ url('Admin/images/prince/'.$one->image)}}" class="img-circle img-responsive"/></div>
                    <h4>{{ $one->name }}</h4>
   ...
   <form method="post" action="{{ url('admin/add_prince_comments') }}">
       @csrf
        @foreach ($princes as $one)
            <input type="hidden" name="prince_id" value="{{ $one->id }}"/>
        @endforeach
