...
<div class="container">
        ...
        @endforeach
         ...
         <div class="comments">
            ...
            @else
                @foreach ($all_comments as $one)
                    <div class="one-comment">
                            <div><img src="{{ url('Admin/images/gayza/'.$one->image)}}" class="img-circle img-responsive"/></div>
                            <h4>{{ $one->name }}</h4>
           ...
           <form method="post" action="{{ url('admin/add_gayza_comments') }}">
                @csrf
                @foreach ($gayza as $one)
                    <input type="hidden" name="gayza_id" value="{{ $one->id }}"/>
                @endforeach
