...
@section('title')
    | @foreach ($gate as $one) {{ $one->title}} @endforeach
@endsection
...
<div class="container">
        ...
        @endforeach
         ...
         <div class="comments">
            ...
            @else
                @foreach ($all_comments as $one)
                    <div class="one-comment">
                            <div><img src="{{ url('Admin/images/gate/'.$one->image)}}" class="img-circle img-responsive"/></div>
                            <h4>{{ $one->name }}</h4>
           ...
           <form method="post" action="{{ url('admin/add_gate_comments') }}">
                @csrf
                @foreach ($gate as $one)
                    <input type="hidden" name="gate_id" value="{{ $one->id }}"/>
                @endforeach
